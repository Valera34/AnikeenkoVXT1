...
function enableButtons () {
	...
	btn.value="Click me for a greeting by period";
	
	btn = document.getElementById("input_greet_by_all");
	btn.onclick=function(){greetByAll();};
	btn.value="Click me for a greeting by Name Surname Period";
}
...
function greetByAll(){
	var name=document.getElementById("name_field").value;
	var surname=document.getElementById("surname_field").value;
	var period=document.getElementById("period_field").value;
	var request = gapi.client.helloworldendpoints.greetByAll({'name': name,'surname': surname, 'period': period});
	request.execute(sayHelloCallback);
}
